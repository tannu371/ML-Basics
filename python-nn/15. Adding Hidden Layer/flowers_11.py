...
for epoch in range(epochs):
    pred_h = [[sum([w * i for w, i in zip(weights, inp)]) + 
...
# We now know that the test sample 21 was the bad prediction. The most active neuron suggests a purple flower. But in fact, the target shows the flower is green.
# Looking ip the bad prediction, and it turns out the point 21 is the green flower which is closer to the purple flowers than to the green flowers.
# We can forgive our network for making this mistake.

# Neural network is not a perfect function emulator. It is a function approximator. You always have to find a good balance between training effort and the result. 
# With neural networks usually good is good enough.