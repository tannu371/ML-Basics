...
# The network was able to predict all the test flowers.
 
# The most complicated thing is to perform all the matrix multiplications and trying to imagine the result the result.
# The fact that our network now has more than one dimension for the weights and inputs does not make it easier.
# And you need pen and paper, even for the most trivial neural networks.
# The only thing that makes it easier over time is practice, practice, practice.

# As always, there is a new problem looking around the corner and the problem is called nonlinear data.