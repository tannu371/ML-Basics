...
# I don’t want to print the weights and bias anymore, so I'll remove the line.
# I'll paste the test data.
...
# And use the data to make test predictions.

test_preds = [predict(i1, i2) for i1, i2 in test_inputs]
for p, t in zip(test_preds, test_targets):
    print(f"target:${t}, predicted:${p:.0f}")

# Let's see how well the network can predict with the normalised values.

# That is very good.
# The predicted values are really close to the target values. 
# Even anomalies like old cars with low mileages are predicted well. As well as young cars with high mileages.
