...
# I let the program run and print the result.
# With some more parameter tweaking, we can improve the recoginition.

# What parameters could be changed?
# You can add some epochs, change the batch size, use the full mnist dataset instead of only 20000 samples.
# You can change the leearning rate, add hidden layer neurons or even add a second hidden layer.

# If you do not get a good rsult immmediately, remember that the weights are initialized randomly. They do not always lead to the best result.
# You migth have to run the training a few times beforre seeing the results you like or store and use the same weights each time you run the system.